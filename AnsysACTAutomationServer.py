...
'''
Description: 
    This script should be copied into the Ansys Workbench Mechanical ACT 
    interface and be used to generate input files and read the Ansys Mechanical
    results files. Additionally, this script will generate the required 
    submission scripts for batch processing locally or on the server. Everything
    has been included in a single file because of the limitations imposed by the 
    Ansys ACT interface.
...
# READ/WRITE FLAG
# 'w' : this will write mechanical APDL input files (.dat) into separate folders
# 'r' : this will read mechanical APDL results files (.rst) after processing. 
RW_FLAG = 'w'

# TEST OR RUN
# 0 : debug         - small number of parameters
# 1 : full          - number of parameters
# 2 : small test    - medium size test, 25 simulation in total
RUN_FLAG = 1
...
# 0 : do nothing
# 1 : write the files for sending, submitting and retrieving from the server
WRITE_SERVER = 1
...
# define frequency and angle vector, each combonation is a unique simulation
# frequency in MHz
...
if RUN_FLAG == 2:
    ...
    # frequency in MHz
    frequency_vector = linspace(0.5,5,5) 
    # angle in degrees
...
# check if input/output directories exist and create
...
for frequency in frequency_vector:
    ...
    mesh_size = lowest_sound_speed / frequency / 6
    # mesh_size = lowest_sound_speed / 1 / 6 # test case
    ...
    for angle in angle_vector:
        ...
        #=======================================================================
        # print some helpful messages
        print('...writing or reading model...')
        
        # solve in interactive mode
        ...
        if RW_FLAG == 'w':
            ...
            if WRITE_SERVER == 1:
                ...
                with open(work_dir + input_dir + "submission_script.sh", "w+b") as f:
                    ...
                    f.write('\nrm $myDir/*.emat $myDir/*.err $myDir/*.esav $myDir/*.full $myDir/*.stat $myDir/*.out $myDir/*.bat $myDir/*.cd $myDir/*.db $myDir/*.DSP')
                ...
                with open(work_dir + input_dir + "retrieve_from_server.sh", 'w+b') as f:
                    ...
                    f.write('\n# get data from server copy to this directories')
        ...
        if RW_FLAG == 'r':
            ...
            # TDO: clear previous results file?
            ExtAPI.DataModel.Project.Model.Analyses[0].Solution.ClearGeneratedData()
